...
// ✅ DEPLOYED BACKEND URL INTEGRATED
...
const CategoryCard = () => {
  ...
  // handleBuyNow with proper scope and propagation block
  const handleBuyNow = (e, product) => {
    e.stopPropagation(); // Stop card click (redirect to detail page)
    
    const quantity = 1;
    // Base size schema reference checks
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 sm:mb-16 gap-6">
          <div className="max-w-2xl text-left">
            <div className="inline-flex items-center gap-2 bg-[#355e3b]/10 text-[#355e3b] px-3.5 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider mb-4">
              <Star size={12} fill="currentColor" />
              Curated Collection
            </div>
            <h2 className="text-3xl sm:text-5xl font-black text-[#2d2a26] tracking-tight leading-tight">
              Our Best Seller <span className="text-[#355e3b] font-serif italic font-normal">Products</span>
            </h2>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[1, 2, 3, 4].map((n, i) => (
              <div key={i} className="animate-pulse space-y-4">
                <div className="bg-gray-100 rounded-[24px] h-[320px] w-full" />
