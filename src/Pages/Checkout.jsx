...
import { FiArrowRight, FiArrowLeft, FiShoppingBag, FiCheckCircle, FiLoader, FiCopy, FiCheck } from 'react-icons/fi';
...
const Checkout = () => {
  ...
  // Professional tracking message modal states
  ...
  // Copy-to-clipboard functionality for professional user experience
  ...
  const handleSubmitOrder = async (e) => {
    ...
    try {
      ...
      // Update the tracking string code state instead of firing standard system alerts
      setPlacedOrderNumber(result.trackingNumber || "HRB-SUCCESSFUL");
    } catch (err) {
  ...
  if (cartItems.length === 0 && !placedOrderNumber) {
    return (
    ...
    <>
      ...
      {placedOrderNumber && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white max-w-md w-full rounded-3xl p-6 sm:p-8 text-center shadow-2xl border border-gray-50 transform scale-100 transition-all">
            <div className="w-16 h-16 bg-[#355e3b]/10 rounded-full flex items-center justify-center mx-auto mb-5 text-[#355e3b]">
              <FiCheckCircle size={36} className="animate-bounce" />
            </div>
            
            <h2 className="text-2xl font-black text-[#2d2a26] mb-2">Order Placed Successfully!</h2>
            <p className="text-sm text-gray-500 mb-6"> Thank you for shopping with Herbalyze. Your natural wellness remedies are now being prepared.</p>
            
            <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 mb-6">
              <span className="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Your Tracking Number</span>
              <div className="flex items-center justify-between bg-white border border-gray-200 rounded-xl p-2.5 px-4 font-mono text-sm font-bold text-gray-800 shadow-xs">
                <span>{placedOrderNumber}</span>
                <button 
                  onClick={() => handleCopyTracking(placedOrderNumber)}
                  className="text-[#355e3b] hover:text-[#2d4d2f] p-1.5 rounded-lg transition-colors flex items-center gap-1 text-xs font-sans font-semibold cursor-pointer"
                  title="Copy Code"
                >
                  {copied ? (
                    <>
            ...
            <button
              onClick={() => navigate("/")}
              className="w-full bg-[#355e3b] text-white py-3.5 rounded-xl font-bold hover:bg-[#2d4d2f] transition shadow-md cursor-pointer text-sm"
            >
              Continue Shopping
            </button>
          </div>
